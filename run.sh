...
if [ -z "$TARGET_YM" ]; then
    ...
    if [ -n "$PARTITION_CHECK" ]; then
        ...
        if hdfs dfs -test -s "${HDFS_RAW_DIR}/BUS_STATION_BOARDING_MONTH_${TARGET_YM}.csv" 2>/dev/null; then
            echo "{$TARGET_YM} 버스 파일이 이미 존재합니다. api 수집을 패스합니다"
        else
            echo "{$TARGET_YM} 버스 파일이 누락되었습니다. api 수집"
            python3.6 src/ingest/collect_bus.py "$TARGET_YM"
        fi
        if hdfs dfs -test -s "${HDFS_RAW_DIR}/weather_data_${TARGET_YM}.csv" 2>/dev/null; then
            echo "{$TARGET_YM} 날씨 파일이 이미 존재합니다. api 수집을 패스합니다"
        else
            echo "{$TARGET_YM} 날씨 파일이 누락되었습니다. api 수집"
            python3.6 src/ingest/collect_weather.py "$TARGET_YM"
        fi
        if hdfs dfs -test -s "${HDFS_RAW_DIR}/dust_data_${TARGET_YM}.csv" 2>/dev/null; then
            echo "{$TARGET_YM} 미세먼지 파일이 이미 존재합니다. api 수집을 패스합니다"
        else
            echo "{$TARGET_YM} 미세먼지 파일이 누락되었습니다. api 수집"
            python3.6 src/ingest/collect_dust_warning.py "$TARGET_YM"
        fi
        if hdfs dfs -test -s "${HDFS_RAW_DIR}/CARD_SUBWAY_MONTH_${TARGET_YM}.csv" 2>/dev/null; then
            echo "{$TARGET_YM} 지하철 파일이 이미 존재합니다. api 수집을 패스합니다"
        else
